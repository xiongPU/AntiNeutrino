...
int FHistFilter(){
   ...
   Int_t i, n_x, n_bin, n_in1bin=200, height=600000;
   Float_t y1, y2,freq_top;
   ...
   TH1F *h1_hi= new TH1F("h1", "auger Data file read", n_bin, xmin, xmax);
   TH1F *h2_hi= new TH1F("h2", "auger Data file read", n_bin, xmin, xmax);
   ...
   ofstream f_hi("auger_hist_compare.txt");
   while(f>>x){
      h2->Fill(x);
   }
   f.close();
   f_hi<<"Bin#\tEnergy\tFreq\tFreq before fix"<<endl;
   freq_top=0;
   for (Int_t k=1; k<=n_bin; k++){
       y1=h1->GetBinContent(k);
       y2=h2->GetBinContent(k);
       if(y1>freq_top) freq_top=y1;
       if(y2>freq_top) freq_top=y2;
       if(y1>=height) {
           h1_hi->SetBinContent(k,y1);
       }
       if(y2>=height) {
           h2_hi->SetBinContent(k,y2);
       }
       if(y1>=height || y2>=height){
           f_hi<<k<<"\t"<<h1->GetBinCenter(k)<<"\t"<<y1<<"\t"<<y2<<endl;
       }
   }
   h1_hi->GetXaxis()->SetRangeUser(xmin, 0.1);
   h1_hi->GetYaxis()->SetRangeUser(0,freq_top*1.1);
   h1_hi->SetLineColor(kRed);
   h1_hi->Draw();
   h2_hi->Draw("same");
   fsave2->Close();
